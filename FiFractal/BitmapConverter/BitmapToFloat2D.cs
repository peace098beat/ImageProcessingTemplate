...
namespace FiFractal
{
    public partial class BitmapConverter
    {
        ...
        /// <returns></returns>
        static public float[,] BitmapToFloat2D(in Bitmap img, int index = 0)
        {
            ...
            // 返却先
            float[,] ResultArray = new float[bmpData.Width, bmpData.Height];
            ...
            for (int y = 0; y < bmpData.Height; y++)
            {
                for (int x = 0; x < bmpData.Width; x++)
                {
                    ...
                    ResultArray[x, y] = (float)(pixels[pos + index] / 255f);
                }
            }
